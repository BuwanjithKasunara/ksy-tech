        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (url.pathname === '/api/projects' && request.method === 'GET') {
                try {
                    ...
                    const { results } = yield env.ksy_db.prepare(query).all();
            ...
            if (url.pathname === '/api/projects' && request.method === 'POST') {
                ...
                try {
                    ...
                    const result = yield env.ksy_db.prepare(`INSERT INTO projects (title, category, description, tags, image_url, github_url, demo_url, is_visible)
           VALUES (?, ?, ?, ?, ?, ?, ?, 1)`).bind(body.title, body.category, body.description, JSON.stringify(body.tags || []), body.imageUrl, body.githubUrl, body.demoUrl || null).run();
            ...
            if (url.pathname.startsWith('/api/projects/') && url.pathname.endsWith('/toggle') && request.method === 'PATCH') {
                ...
                const id = parts[3];
                yield env.ksy_db.prepare(`UPDATE projects SET is_visible = CASE WHEN is_visible = 0 THEN 1 ELSE 0 END WHERE id = ?`).bind(id).run();
          ...
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`).bind(body.id, body.name, body.role, body.description || null, body.image || null, body.avatarIcon || null, body.avatarZoom || null, body.isFounder ? 1 : 0, body.github || null, body.linkedin || null, body.email || null, body.phone || null, body.education || null, JSON.stringify(body.keySkills || []), JSON.stringify(body.experienceHighlights || []), body.cvPdfUrl || null).run();
                    return jsonResponse({ success: true });
                }
                catch (err) {
                    return jsonResponse({ error: err.message }, 500);
            ...
            if (url.pathname.startsWith('/api/team/') && url.pathname.endsWith('/toggle') && request.method === 'PATCH') {
                ...
                const parts = url.pathname.split('/');
                const id = parts[3];
                yield env.ksy_db.prepare(`UPDATE team_members SET is_visible = CASE WHEN is_visible = 0 THEN 1 ELSE 0 END WHERE id = ?`).bind(id).run();
                return jsonResponse({ success: true });
            }
